...
public class Flying_Reaper : MonoBehaviour
{
    ...
    void FixedUpdate() 
    {
        ...
        if (eH.hp > 0 && eH.freezeTimer <= 0)
            transform.position = Vector3.Lerp(transform.position, randomPosition, Time.deltaTime * speed * speedModifier * speedMult);
    }
    ...
    private IEnumerator movement() 
    {
        ...
        waiting = true;
        yield return new WaitForSeconds(moveDelay/speedMult);
        waiting = false;
    }
